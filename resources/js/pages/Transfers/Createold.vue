...
const form = useForm({
    from_store_id: props.selectedSource || '',
    to_store_id: props.selectedDestination || '',
...
const availabilityResults = ref<Record<number, { available: number; status: 'ok' | 'low' | 'insufficient' }>>({})
...
const removeItem = (index: number) => {
    if (form.items.length > 1) {
        ...
        // Recheck availability after removing item
        checkAvailability()
    }
}
...
const checkAvailabilityold = async () => {
    if (!form.from_store_id || form.items.length === 0) return
    ...
    try {
        ...
        availabilityResults.value = result.data || {}
    } catch (error) {
...
const checkAvailability = async () => {
    ...
    try {
        ...
        if (response.data.insufficient_items?.length > 0) {
            response.data.insufficient_items.forEach(item => {
...
const getStockStatusMessage = (productId: number, requestedQty: number) => {
    ...
    if (availability.status === 'insufficient') {
        return {
            message: `Insufficient stock. Available: ${availability.available}`,
            class: 'text-red-600'
        }
    }
    if (availability.status === 'low') {
        return {
            message: `Low stock. Available: ${availability.available}`,
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        >
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    >
                                        ...
                                        <option
                                            ...
                                            :value="product.id"
                                        >
                                            {{ product.name }} ({{ product.sku }}) - ${{ product.selling_price }} per {{ product.unit }}
                                        </option>
                                    </select>
                            ...
                            <div v-if="item.product_id && form.from_store_id" class="mt-2">
                                ...
                                <div v-else-if="getStockStatusMessage(item.product_id, item.quantity)" 
                                     :class="['text-sm', getStockStatusMessage(item.product_id, item.quantity)?.class]">
                                    {{ getStockStatusMessage(item.product_id, item.quantity)?.message }}
                                </div>
                            </div>

                            <!-- Product Details -->
                            <div v-if="item.product_id" class="mt-2 text-xs text-gray-500">
                                <div class="flex items-center">
                                    <InformationCircleIcon class="w-3 h-3 mr-1" />
                                    <span>Unit: {{ getProductDetails(Number(item.product_id))?.unit }} | 
                                           Price: ${{ getProductDetails(Number(item.product_id))?.selling_price }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
